/* infobox.c  -- 
 * $Id: infobox.c,v 1.1 2001/10/20 03:05:49 bitman Exp $
...
void editboardinfo(world* myworld, int curboard, displaymethod* d)
{
	...
	do {
		...
		switch (key) {
			...
			case DKEY_DOWN:
				if (curoption < BRDINFO_BRDWEST) curoption++;
				break;
												...
												int cursorx, int cursory, displaymethod* d)
{
	...
	switch (curoption) {
		case BRDINFO_TITLE:
			...
			if (line_editor(13, cursory, 0x0f, 0x00, buffer, 34, LINED_NORMAL, d)) {
		...
		case BRDINFO_MAXSHOTS:
			...
			if (line_editor(cursorx, cursory, 0x0f, 0x00, buffer, 3,
		...
		case BRDINFO_TIMELIM:
			...
			if (line_editor(cursorx, cursory, 0x0f, 0x00, buffer, 5,
											LINED_NOALPHA | LINED_NOPUNCT | LINED_NOSPACES, d)) {
				long int timelimit;
